...
function getAllowOriginHeader(origin: string | undefined) {
    switch (origin) {
        case "http://localhost:5173":
        case "https://qa-pup.pages.dev":
            return origin;
        default:
            return "https://qa-pup.cloudydaiyz.com"
    }
}
...
export const handler: APIGatewayProxyHandler = async (event, context) => {
    ...
    const headers = {
        "Access-Control-Allow-Origin": getAllowOriginHeader(event.headers.origin),
